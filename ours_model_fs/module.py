...
def latentEncodeScaleNet(input_code, options, reuse=False, name="latentEncodeScaleNet"):
...
def latentDecodeScaleNet(input_code, options, reuse=False, name="latentDecodeScaleNet"):
...
def encoder_resnet(image, latentscale, options, reuse=False, name="encoder"):
    ...
    with tf.variable_scope(name):
        ...
        def residule_block(x, ls1, ls2, dim, ks=3, s=1, name='encoder_res'):
            p = int((ks - 1) / 2)
            y = tf.pad(x, [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
            y = instance_norm(tf.multiply(conv2d(y, dim, ks, s, padding='VALID', name=name+'_c1'), ls1), name+'_bn1')
            y = tf.pad(tf.nn.relu(y), [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
            y = instance_norm(tf.multiply(conv2d(y, dim, ks, s, padding='VALID', name=name+'_c2'), ls2), name+'_bn2')
            return y + x
        ...
        c1 = tf.nn.relu(instance_norm(tf.multiply(conv2d(c0, options.gf_dim, 7, 1, padding='VALID', name='encoder_e1_c'), latentscale['hc1']), 'encoder_e1_bn'))
        c2 = tf.nn.relu(instance_norm(tf.multiply(conv2d(c1, options.gf_dim*2, 3, 2, name='encoder_e2_c'), latentscale['hc2']), 'encoder_e2_bn'))
        c3 = tf.nn.relu(instance_norm(tf.multiply(conv2d(c2, options.gf_dim*4, 3, 2, name='encoder_e3_c'), latentscale['hc3']), 'encoder_e3_bn'))
        # define G network with 9 resnet blocks
        r1 = residule_block(c3, latentscale['hr11'], latentscale['hr12'], options.gf_dim*4, name='encoder_r1')
        r2 = residule_block(r1, latentscale['hr21'], latentscale['hr22'], options.gf_dim*4, name='encoder_r2')
        r3 = residule_block(r2, latentscale['hr31'], latentscale['hr32'], options.gf_dim*4, name='encoder_r3')
        r4 = residule_block(r3, latentscale['hr41'], latentscale['hr42'], options.gf_dim*4, name='encoder_r4')
        r5 = residule_block(r4, latentscale['hr51'], latentscale['hr52'], options.gf_dim*4, name='encoder_r5')
        
        return r5


def decoder_resnet(r5, latentscale, options, reuse=False, name="decoder"):
    ...
    with tf.variable_scope(name):
        ...
        def residule_block(x, ls1, ls2, dim, ks=3, s=1, name='decoder_res'):
            p = int((ks - 1) / 2)
            y = tf.pad(x, [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
            y = instance_norm(tf.multiply(conv2d(y, dim, ks, s, padding='VALID', name=name+'_c1'), ls1), name+'_bn1')
            y = tf.pad(tf.nn.relu(y), [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
        ...
        # The network with 9 blocks consists of: c7s1-32, d64, d128, R128, R128, R128,
        # R128, R128, R128, R128, R128, R128, u64, u32, c7s1-3
        r6 = residule_block(r5, latentscale['hr61'], latentscale['hr62'], options.gf_dim*4, name='decoder_r5')
        r7 = residule_block(r6, latentscale['hr71'], latentscale['hr72'], options.gf_dim*4, name='decoder_r4')
        r8 = residule_block(r7, latentscale['hr81'], latentscale['hr82'], options.gf_dim*4, name='decoder_r3')
        r9 = residule_block(r8, latentscale['hr91'], latentscale['hr92'], options.gf_dim*4, name='decoder_r2')

        d1 = tf.multiply(deconv2d(r9, options.gf_dim*2, 3, 2, name='decoder_d1_dc'), latentscale['hd1'])
        d1 = tf.nn.relu(instance_norm(d1, 'decoder_d1_bn'))
        d2 = tf.multiply(deconv2d(d1, options.gf_dim, 3, 2, name='decoder_d2_dc'), latentscale['hd2'])
        d2 = tf.nn.relu(instance_norm(d2, 'decoder_d2_bn'))
